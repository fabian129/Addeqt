  ...
  return (
    ...
    >
      <div className="max-w-7xl mx-auto px-6 md:px-8 py-4 flex items-center justify-between">
        {/* Logo + Nav pill */}
        <div
          className={`flex items-center gap-1 rounded-full px-1 py-1 transition-all duration-500 ${
            scrolled
        ...
        {/* Right pill: email + CTA */}
        <div
          className={`flex items-center gap-2 rounded-full px-1 py-1 transition-all duration-500 ${
            scrolled
              ? "bg-gray-100 ring-1 ring-gray-200"
              : "bg-white/[0.08] ring-1 ring-white/10 backdrop-blur-sm"
          }`}
        >
          <Link
            ...
            className={`hidden sm:inline-flex items-center gap-2 text-sm font-medium rounded-full px-4 py-2 transition ${
              scrolled
                ? "text-gray-500 hover:text-gray-900 hover:bg-gray-200"
                : "text-white/80 hover:text-white hover:bg-white/10"
            }`}
          ...
          <Link
            ...
            className={`inline-flex items-center gap-2 text-sm font-medium rounded-full px-4 py-2 transition ${
              scrolled
                ? "text-white bg-[#242748] hover:bg-[#1a1d36]"
        ...
        >
          ...
          <span className={`block w-5 h-[1.5px] bg-current transition-all duration-300 ${menuOpen ? "-rotate-45 -translate-y-[1.5px]" : ""}`} />
        </button>
      </div>
      ...
      {menuOpen && (
        <div className="md:hidden bg-white/95 backdrop-blur-xl border-t border-black/[0.06] px-6 py-8 space-y-6">
          <Link href="#om-oss" onClick={() => setMenuOpen(false)} className="block text-sm text-gray-600 hover:text-[#1d1d1f]">Om oss</Link>
          <Link href="#tjanster" onClick={() => setMenuOpen(false)} className="block text-sm text-gray-600 hover:text-[#1d1d1f]">Tjänster</Link>
          <Link href="#kontakt" onClick={() => setMenuOpen(false)} className="block bg-[#242748] text-white px-5 py-3 rounded-full text-sm font-medium text-center">Boka möte</Link>
        </div>
      )}
    </header>
  );
